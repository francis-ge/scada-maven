...
public class AjaxMainRecodeAction extends ActionSupport {
	...
	public String mainRecodeForOneHistoryCurve(){
	...
	public String mainRecodeForOne(){
		...
		String hql ="from MainRecode mr where mr.fun.id=? AND mr.dateTime>? AND mr.dateTime<?";
		
		try {
			dataList = recodeService.findMapByHqlPaging(hql, (page-1)*rows, rows, funId, beginTime, endTime);
			List<Map<String, Object>> dataList1 = recodeService.findMapByHql("select new map(count(*) as total) from MainRecode mr where mr.fun.id=? and mr.dateTime>? AND mr.dateTime<? ", funId, beginTime, endTime);
			long total = (long) (dataList1.get(0).get("total"));
			message = "查询成功！";
