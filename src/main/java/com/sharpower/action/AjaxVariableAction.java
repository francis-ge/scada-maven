...
public class AjaxVariableAction extends ActionSupport {
	...
	public String allVariableQuick(){
		try {
			String totalHql = "";
			Long total;
			
			if (searchKey==null) {
				String hql = "select new Variable(v.id,v.name,v.dbName,v.showNameCN, v.type.id, v.type.plcType, v.type.type, v.plcType.id,v.plcType.name) from Variable v";
				variables = variableService.findEntityByHQLPaging(hql, (page-1)*rows, rows);
				//variables = variableService.findAllEntitiesPaging((page-1)*rows, rows);
				total = (Long) variableService.uniqueResult("SELECT count(*) From Variable");
			}else{
				//String hql = "From Variable v WHERE v.name like ?";
				String hql = "select new Variable(v.id,v.name,v.dbName,v.showNameCN,v.type.plcType,v.type.type,v.plcType.name) from Variable v where v.name like ?";
				variables = variableService.findEntityByHQLPaging(hql, (page-1)*rows, rows, "%"+searchKey+"%");
