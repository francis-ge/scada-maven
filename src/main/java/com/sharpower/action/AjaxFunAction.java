...
public class AjaxFunAction extends ActionSupport {
	...
	public String allFunQuick(){
		try {
			String hql = "select new Fun(f.id, f.name, f.plcType.id, f.plcType.name, f.address, f.line) from Fun f";
			funs = funService.findEntityByHQL(hql);
			result = "共查到" + funs.size() + "条记录。";
		} catch (Exception e) {
			result = "获取失败！"+e.getMessage();
		}
