...
public class FunTroubleBeckhoffService implements FunTroubleVariableReader{
	private PlcTypeService plcTypeService;
	private Map<String, FunTroubleBeckhoff> funDataReaders = new HashMap<>();
	...
	@Override
	public Map<FunTroubleVariable, Object> readDataAll(Fun fun) throws AdsException {
		if (funDataReaders.isEmpty()) {
			List<PlcType> plcTypes = this.plcTypeService.findEntityByHQL("FROM PlcType p LEFT JOIN FETCH p.funTroubleVariables WHERE p.plcCommType.name='beckhoff'");
			
			for (PlcType plcType : plcTypes) {
				FunTroubleBeckhoff funTroubleBeckhoff = new FunTroubleBeckhoff(plcType.getFunTroubleVariables());
				funDataReaders.put(plcType.getName(),funTroubleBeckhoff);
			}
		}
		
		return funDataReaders.get(fun.getPlcType().getName()).readData(fun);
	}
	
	class FunTroubleBeckhoff{
		...
		private  List<FunTroubleVariable> valNames = new ArrayList<>();//变量名集合
		 ...
		 */
		public Map<FunTroubleVariable, Object> readData(Fun fun) throws AdsException {
		...
		public FunTroubleBeckhoff(List<FunTroubleVariable> funTroubleVariables) {
			this.valNames = funTroubleVariables;
			readyToReadData();
		}
